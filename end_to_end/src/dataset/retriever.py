...
def collate_retriever(data, device=None):
    sample = data[0]
    if device is None:
        device = torch.device('cuda:0')
    h_id_tensor = torch.tensor(sample['h_id_list'], device=device)
    r_id_tensor = torch.tensor(sample['r_id_list'], device=device)
    t_id_tensor = torch.tensor(sample['t_id_list'], device=device)
    # 나머지는 이미 tensor면 x.to(device), numpy/리스트면 torch.tensor(x, device=device)
    q_emb = sample['q_emb'].to(device) if torch.is_tensor(sample['q_emb']) else torch.tensor(sample['q_emb'], device=device)
    entity_embs = sample['entity_embs'].to(device) if torch.is_tensor(sample['entity_embs']) else torch.tensor(sample['entity_embs'], device=device)
    relation_embs = sample['relation_embs'].to(device) if torch.is_tensor(sample['relation_embs']) else torch.tensor(sample['relation_embs'], device=device)
    topic_entity_one_hot = sample['topic_entity_one_hot'].to(device) if torch.is_tensor(sample['topic_entity_one_hot']) else torch.tensor(sample['topic_entity_one_hot'], device=device)
    target_triple_probs = sample['target_triple_probs'].to(device) if torch.is_tensor(sample['target_triple_probs']) else torch.tensor(sample['target_triple_probs'], device=device)

    return (
        h_id_tensor, r_id_tensor, t_id_tensor, q_emb,
        entity_embs, len(sample['non_text_entity_list']),
        relation_embs, topic_entity_one_hot,
        target_triple_probs, sample['a_entity_id_list']
    )
